...
function createComment(url, request) {
  ...
  const response = {};
  if (requestComment && requestComment.username && requestComment.body && database.users[requestComment.username] &&
...
function updateComment(url, request) {
  ...
  if (!id || !request.body || !request.body.comment) {
    response.status = 400;
    ...
    const requestComment = request.body && request.body.comment;
    if (requestComment.body) savedComment.body = requestComment.body;
    // the upvotes and downvotes were made based on the old comment text, so reset them
    savedComment.upvotedBy = [];
    savedComment.downvotedBy = [];
  ...
  if (!id) {
    response.status = 404;
      ...
      if (! savedComment) {
        ...
        // delete the comment from the appropriate article's comments array
        const commentArticle = database.articles[savedComment.articleId];
        commentArticle.commentIds.splice(commentArticle.commentIds.indexOf(id), 1);

        // remove the comment from the appropriate user's comments array
